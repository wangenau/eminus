...
def test_pyscf_mgga(xc, Nspin):
    ...
    dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    tau = n_spin
    e_out, v_out, vsigma_out, vtau_out = pyscf_functional(xc, n_spin, Nspin, dn_spin=dn_spin,
                                                          tau=tau)
    e_test, v_test, vsigma_test, vtau_test = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin,
                                                              tau=tau)
    assert_allclose(e_out, e_test)
    assert_allclose(v_out, v_test)
    assert_allclose(vsigma_out, vsigma_test)
