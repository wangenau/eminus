...
def test_libxc_functional_exc(xc, Nspin):
    ...
    e_out, _, _ = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
...
def test_libxc_functional_vxc(xc, Nspin):
    ...
    if is_gga(xc):
        dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    _, v_out, _ = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
    v_test, _ = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)
...
def test_libxc_functional_vsigmaxc(xc, Nspin):
    ...
    n_spin = n_tests[Nspin]
    dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    _, _, vsigma_out = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
    _, vsigma_test = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)
...
def test_pyscf_functional_exc(xc, Nspin):
    ...
    if is_gga(xc):
        dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    e_out, _, _ = pyscf_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
    e_test = get_exc(xc, n_spin, Nspin, dn_spin=dn_spin)
    assert_allclose(e_out, e_test)
...
def test_pyscf_functional_vxc(xc, Nspin):
    ...
    if is_gga(xc):
        dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    _, v_out, _ = pyscf_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
    v_test, _ = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)
    assert_allclose(v_out, v_test)
...
def test_pyscf_functional_vsigmaxc(xc, Nspin):
    ...
    n_spin = n_tests[Nspin]
    dn_spin = np.stack([n_spin, n_spin, n_spin], axis=2)
    _, _, vsigma_out = pyscf_functional(xc, n_spin, Nspin, dn_spin=dn_spin)
    _, vsigma_test = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)
    assert_allclose(vsigma_out, vsigma_test)
