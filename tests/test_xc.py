...
def test_get_exc(xc, Nspin):
    ...
    e_test, _, _, _ = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)  # type: ignore [call-overload]
...
def test_get_vxc(xc, Nspin):
    ...
    if is_gga(xc):
        dn_spin = np.stack((n_spin, n_spin, n_spin), axis=2)
    v_out, _, _ = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)  # type: ignore [call-overload]
    _, v_test, _, _ = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)  # type: ignore [call-overload]
...
def test_get_vsigmaxc(xc, Nspin):
    ...
    n_spin = n_tests[Nspin]
    dn_spin = np.stack((n_spin, n_spin, n_spin), axis=2)
    _, vsigma_out, _ = get_vxc(xc, n_spin, Nspin, dn_spin=dn_spin)  # type: ignore [call-overload]
    _, _, vsigma_test, _ = libxc_functional(xc, n_spin, Nspin, dn_spin=dn_spin)  # type: ignore [call-overload]
