...
def cubegen(atoms, field, filename):
    ...
    # (triple loop over z,y,x instead of x,y,z), so rearrange it.
    idx = []
    for x in range(S[0]):
        for y in range(S[1]):
            for z in range(S[2]):
                idx.append(x + y * S[0] + z * S[0] * S[1])
    idx = np.asarray(idx)
    field = field[idx]
    ...
    with open(filename, 'w') as f:
        ...
        # Number of atoms at first (int), and origin of the coordinate system (float)
        ...
        # We only have a cubic box, so the vector only has one non-zero component
        ...
        for i in range(S[0] * S[1]):
            # Print every round of z-values, so we can add empty lines between them
            data_str = '%+1.5e  ' * S[2] % tuple(field[i * S[2]:(i + 1) * S[2]])
            # Print 6 values of data per row
