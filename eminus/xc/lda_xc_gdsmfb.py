...
def get_dbdtheta(theta,b1,b2,b3,b4,b5):
    ...
    tmp11 = (2*b4*theta + 4*b5*theta**3)*(b1 + b2*theta**2 + b3*theta**4)*np.tanh(1/np.sqrt(theta))
    tmp12 = (b4*theta**2 + b5*theta**4 + 1)**2
    tmp2 = tmp11/tmp12
    with np.errstate(over="ignore"):
        tmp21 = (b1 + b2*theta**2 + b3*theta**4)
        tmp22 = (2*(b4*theta**2 + b5*theta**4 + 1)*theta**(3/2)*np.cosh(1/np.sqrt(theta))**2)
        tmp23 = tmp21/tmp22
...
def get_e(theta,e1,e2,e3,e4,e5):
    """Get e."""
    return np.tanh(1/theta)* (e1 + e2*theta**2 + e3*theta**4) / (1 + e4*theta**2 + e5*theta**4)

def get_dedtheta(theta,e1,e2,e3,e4,e5):
    """Get de / dtheta."""
    tmp1 = (2*e2*theta + 4*e3*theta**3)*np.tanh(1/theta)/(e4*theta**2 + e5*theta**4 + 1)
    tmp11 = (2*e4*theta + 4*e5*theta**3)*(e1 + e2*theta**2 + e3*theta**4)*np.tanh(1/theta)
...
def get_dcdtheta(theta,c1,c2,e1,e2,e3,e4,e5):
    ...
    tmp31 = (2*e4*theta + 4*e5*theta**3)*(e1 + e2*theta**2 + e3*theta**4)*np.tanh(1/theta)
...
def get_dddtheta(theta,d1,d2,d3,d4,d5):
    ...
    tmp11 = (2*d4*theta + 4*d5*theta**3)*(d1 + d2*theta**2 + d3*theta**4)*np.tanh(1/np.sqrt(theta))
    tmp12 = (d4*theta**2 + d5*theta**4 + 1)**2
    tmp2 = tmp11/tmp12
    with np.errstate(over="ignore"):
        tmp21 = (d1 + d2*theta**2 + d3*theta**4)
        tmp22 = (2*(d4*theta**2 + d5*theta**4 + 1)*theta**(3/2)*np.cosh(1/np.sqrt(theta))**2)
        tmp23 = tmp21/tmp22
        tmp3 = np.where(theta < 0.001,0,tmp23)
    return tmp1 - tmp2 - tmp3
